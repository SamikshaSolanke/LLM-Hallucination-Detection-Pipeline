...
def plot_hallucination_by_category(by_category: pd.DataFrame) -> Path:
    ...
    for cat in categories:
        ...
        f = subset[subset["model"] == "llama-3.1-8b-instant"]["hallucination_rate"]
        p = subset[subset["model"] == "llama-3.3-70b-versatile"]["hallucination_rate"]
    ...
    ax.barh(y + bar_h / 2, flash_rates, bar_h, label="Llama 3.1 8B (Flash)", color=FLASH_COLOR, alpha=0.88)
    ax.barh(y - bar_h / 2, pro_rates,   bar_h, label="Llama 3.3 70B (Pro)",   color=PRO_COLOR,   alpha=0.88)

    ax.set_yticks(y)
    ax.set_yticklabels(categories, fontsize=9)
    ax.invert_yaxis()
    ax.set_xlabel("Hallucination Rate (%)")
    ax.set_title("Hallucination Rate by Category — Llama 8B vs Llama 70B", fontsize=14, pad=14)
...
def plot_model_comparison(overall: pd.DataFrame) -> Path:
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([m.replace("llama-3.1-8b-instant", "Llama 8B").replace("llama-3.3-70b-versatile", "Llama 70B") for m in models], fontsize=11)
    ax.set_ylabel("Rate (%)")
    ax.set_ylim(0, 110)
    ax.set_title("Overall Model Performance — Llama 8B vs Llama 70B", fontsize=14, pad=14)
    ax.legend()
...
def plot_delta_chart(delta: pd.DataFrame, top_n: int = 20) -> Path:
    ...
    ax.set_xlabel("Hallucination Rate Delta (8B − 70B)  %")
    ax.set_title(f"Llama 8B vs 70B — Biggest Category Gaps (Top {top_n})", fontsize=14, pad=14)

    legend_elements = [
        mpatches.Patch(color=PRO_COLOR,   label="70B wins (8B hallucinates more)"),
        mpatches.Patch(color=FLASH_COLOR, label="8B wins (70B hallucinates more)"),
    ]
...
def generate_html_report(report: dict, chart_paths: dict) -> Path:
    ...
    flash_row = overall[overall["model"] == "llama-3.1-8b-instant"].iloc[0]
    pro_row   = overall[overall["model"] == "llama-3.3-70b-versatile"].iloc[0]
...
<header>
  <div class="container">
    ...
    <p>TruthfulQA MCQ1 · 817 questions · 38 categories · Llama 3.1 8B vs Llama 3.3 70B</p>
  </div>
</header>
...
<div class="container">
  ...
  <div class="kpi-grid">
    <div class="kpi">
      ...
      <div class="lbl">Llama 8B Accuracy</div>
    </div>
    <div class="kpi">
      <div class="val red">{flash_row['hallucination_rate']}%</div>
      <div class="lbl">Llama 8B Hallucination Rate</div>
    </div>
    <div class="kpi">
      <div class="val green">{pro_row['accuracy_rate']}%</div>
      <div class="lbl">Llama 70B Accuracy</div>
    </div>
    <div class="kpi">
      <div class="val red">{pro_row['hallucination_rate']}%</div>
      <div class="lbl">Llama 70B Hallucination Rate</div>
    </div>
  </div>
...
<footer>
  <div class="container">
    <p>Hallucination Detection Pipeline · TruthfulQA · Llama 3.1 8B & Llama 3.3 70B via Groq</p>
  </div>
</footer>
</body>
</html>"""
